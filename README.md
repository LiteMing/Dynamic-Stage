...
LOD visibility and live LOD package or Flight replacements can be instant, fade, or blur transitions. Persistent blur is independent from transitions and uses a `0..32` pixel radius; `0` disables it. DH near fade remains configurable. Voxy keeps its native `8/16` near plane and far-depth precision. With `voxy-culling=false`, the matching HDRS Voxy build preserves camera-adjacent LOD sections and clamps only geometry entering the near plane just inside that plane, avoiding both whole-section near clipping and the distant Z-buffer stripes caused by a global `0.1` near plane. This stage-only override leaves normal-world Voxy rendering unchanged. Dynamic Stage filters only the native backend's intermediate LOD color texture before DH or Voxy performs its original depth-aware composite, so the sky, stage blocks, entities, and UI are not blurred.
